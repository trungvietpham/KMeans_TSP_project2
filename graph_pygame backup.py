...
class Node(object):
    ...
    creation_counter = 0
    def __init__(self, color, size_rect = NODESIZE):
...
def create_button(rect, text, font_color = (0,0,0), text_color = (255,255,255)):
    global SCREEN
    color_dark = (100,100,100)
    pygame.draw.rect(SCREEN, font_color, rect)
    smallfont = pygame.font.SysFont('Corbel',16) 
    t = smallfont.render(text , True , text_color)
...
def main():
        ...
        if len(cluster_data[key1]['child_cluster_list'])>1:
            for key2 in TSP_data[key1]["child_cluster_list"]:
                ...
                for i in range(1, len(route)):
                    ...
                    color_list_full_node.append(BGCOLOR)
    ...
    for i in range(len(color_list_full_node)):
        ...
        if color_list_full_node[i] == BGCOLOR: 
            correlate_all_node[i][i-1] = 1
            if color_list_full_node[i+1] in [red, blue, orange]:
    ...
    #         if len(cluster_data[key1]['child_cluster_list'])>1:
    #             i+=1
    #             correlate_all_node[k][i] = 1
            
    #         route = TSP_data[key1]["child_cluster_list"][key2].split(' -> ')[:-1]
    #         j=0
    #         for key3 in range(len(route)):
    #             i+=1
    #             j+=1
    #             correlate_all_node[i][i-1] = 1
    #         correlate_all_node[i][i-j+1] = 1
    #     i+=1
    ...
    graph_all_node = create_graph(convert_lat_long_to_descartes(full_node_list, 0+offset[0],SIZE[0]-offset[0], 0+offset[1], SIZE[1]-offset[1]), color_list_full_node, correlate_all_node)
