...
def Pre_TSP_phase(n_node_threshold, vehicle_fname):
    ...
    print('\t\tTotal distance = {} (km)'.format(round(np.sum(np.sum(total_distance)), 3)))
    ...
    summary.append('\t\tTotal distance = {} (km)'.format(round(np.sum(np.sum(total_distance)), 3)))
    ...
    for i in range(n_clusters):
        ...
        details.append('\t\t\tTotal distance in all cluster child: {} (km)'.format(round(total_distance[i], 3)))
