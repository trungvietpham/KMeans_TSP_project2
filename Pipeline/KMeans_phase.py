...
def KMeans_phase(vehicle_fname):
    ...
    time1 = time.time()
    (centers, labels, it, cluster_list, total_distance) = model.fit(optimizer=optimizer, city_list = city_list,capacity_array = capacity_array, scale_coef=scale_coef_list, distance_coef=convert_coef, normalization_flag=False, alpha=5*n_vehicles, penalty_coef=10, zeros_penalty=1000000, shuffle=True, epsilon=1e-3)
    time2 = time.time()
